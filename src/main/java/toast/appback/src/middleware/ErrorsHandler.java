...
public class ErrorsHandler {
    ...
    public static void handleErrors(List<? extends IError> error) {
        ...
        System.out.println("Handling " + error.size() + " errors");
        if (error.size() == 1) {
            throw new ApplicationException(handleSingleError(error.getFirst()));
        }
        throw new ApplicationException(handleMultipleErrors(error));
    }

    public static void handleError(IError error) throws ApplicationException {
        throw new ApplicationException(handleSingleError(error));
    }
    
    private static ErrorData handleSingleError(IError error) {
