...
import toast.appback.src.debts.application.usecase.implementation.RejectDebtUseCase;
...
public class RejectDebtTest {

    private DebtRepository debtRepository;
    private UserRepository userRepository;
    private DomainEventBus domainEventBus;

    private RejectDebtUseCase useCase;
    ...
    void setup() {
        ...
        useCase = new RejectDebtUseCase(debtRepository, userRepository, domainEventBus);

        COMMAND = new EditDebtStatusCommand(DEBT_ID, DEBTOR_ID);

        Context ctx = Context.load("Purpose", "Desc");
        DebtMoney money = DebtMoney.load(new BigDecimal("10.00"), "MXN");

        debt = DebtBetweenUsers.load(
                DEBT_ID, ctx, money, DEBTOR_ID, CREDITOR_ID, Status.PENDING
        );
    ...
    void execute_ShouldThrow_WhenActorIsNotDebtor() {
        ...
        EditDebtStatusCommand wrong =
                new EditDebtStatusCommand(DEBT_ID, CREDITOR_ID); // creditor intentando rechazar

        assertThrows(UnauthorizedActionException.class,
                () -> useCase.execute(wrong));
    }
    ...
    @Test
    void execute_ShouldRejectDebt_Save_AndPublishEvents() {
        ...
        assertEquals(Status.REJECTED.toString(), view.status());
