...
public class ResendDebtTest {

    private DebtRepository debtRepository;
    private UserRepository userRepository;
    private DomainEventBus domainEventBus;

    private ResendDebtUseCase useCase;
    ...
    void setup() {
        ...
        useCase = new ResendDebtUseCase(debtRepository, userRepository, domainEventBus);

        COMMAND = new EditDebtStatusCommand(DEBT_ID, CREDITOR_ID);

        Context ctx = Context.load("Purpose", "Desc");
        DebtMoney money = DebtMoney.load(new BigDecimal("100.00"), "MXN");

        debt = DebtBetweenUsers.load(
                DEBT_ID, ctx, money, DEBTOR_ID, CREDITOR_ID, Status.REJECTED
        );

        debtorUser = mock(User.class);
        when(debtorUser.getUserId()).thenReturn(DEBTOR_ID);
        when(debtorUser.getName()).thenReturn(Name.load("Carlos", "Lopez"));
    ...
    @Test
    void execute_ShouldResendDebt_Save_AndPublishEvents() {
        ...
        assertEquals("Carlos", view.debtorSummary().userFirstName());
        assertEquals("Laura", view.creditorSummary().userFirstName());
        assertEquals(debt.getStatus().toString(), view.status());
